...
class PostgreSQLIfcGeometryDataRepository(IfcGeometryDataRepository):
    ...
    def _put_glb(self, ifc_model_id: IfcModelId, geometries: List[IfcGeometryDataDTO]):
        ...
        with self._get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute("""INSERT INTO ifcgeometryglb (ifcmodel_id, glb_normal, glb_lod2, glb_lod1) VALUES (%s, %s, %s, %s)""", (ifc_model_id, glbs[0], glbs[1], glbs[2]))
                conn.commit()
...
class PostgreSQLIfcGeometryDataDAO(IfcGeometryDataDAO):
    ...
    def find_glb_by_ifcmodel_id(self, ifcmodel_id: str, lod: int) -> bytes:
        """"""
        if lod == 1:
            query = "SELECT glb_lod1 FROM ifcgeometryglb WHERE ifcmodel_id = %s"
        elif lod == 2:
            query = "SELECT glb_lod2 FROM ifcgeometryglb WHERE ifcmodel_id = %s"
        else:
